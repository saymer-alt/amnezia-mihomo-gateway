...
Новый TUN-стек Mihomo `mips` был проверен на трёх VPS. На сервере **EE** отдельно выполнены три сравнительных теста при переключении `gvisor → mips`. Во всех этих проверках `mips` показал результат хуже текущего `gvisor`-baseline.
